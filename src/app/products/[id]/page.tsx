    ...
    <div className="min-h-screen pt-32 pb-20 px-6 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div className="space-y-6">
          <div className="relative aspect-square rounded-[40px] overflow-hidden glass-morphism group">
             <img 
             ...
             {product.images.length > 1 && (
               <div className="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity">
                 <button 
                   ...
                   className="w-12 h-12 bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center text-white"
                 >
                   <ChevronLeft size={24} />
                 </button>
                 <button 
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4 py-8 border-y border-white/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Condition</div>
                <div className="font-bold">{product.condition}</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-primary">
              ...
              <div>
                ...
                <div className="font-bold">{product.location || "On Campus"}</div>
              </div>
            </div>
          </div>

          {/* Seller Info */}
          <div className="glass-morphism p-6 rounded-[32px] flex items-center justify-between group">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-secondary rounded-2xl overflow-hidden">
              ...
              <div>
                <div className="text-xs text-gray-500 font-bold uppercase tracking-widest mb-0.5">Posted by</div>
                <div className="font-bold text-lg">{product.seller.name}</div>
                <div className="text-xs text-primary font-medium">{product.seller.college}</div>
              </div>
            </div>
            <Link href={`/profile/${product.seller._id}`}>
              <button className="p-3 bg-white/5 rounded-xl group-hover:bg-primary/20 group-hover:text-primary transition-all">
          ...
          <div className="flex gap-4 pt-4">
            <button 
              onClick={handleChat}
              className="flex-[2] bg-primary hover:bg-primary-hover py-5 rounded-3xl font-black flex items-center justify-center gap-2 shadow-xl shadow-primary/20 transition-all"
            >
              <MessageCircle size={22} /> Start Chat
            </button>
            <button 
              onClick={handleWishlist}
              className={`flex-1 border py-5 rounded-3xl font-bold flex items-center justify-center gap-2 transition-all ${
                isWishlisted 
            ...
            <button 
              onClick={handleShare}
              className="w-16 bg-white/5 border border-white/10 hover:bg-white/10 rounded-3xl flex items-center justify-center transition-all"
            >
              <Share2 size={22} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500 justify-center">
            <ShieldCheck size={14} className="text-green-500" /> Secure Campus Trade Policy Active
          </div>
        </div>
      </div>
    </div>
...
// Re-using ArrowUpRight icon from lucide-react (already imported as ArrowUpRight)
