...
export default function GoogleSignInButton({ mode = "login" }: { mode?: "login" | "register" }) {
  ...
  if (!clientId) {
    return (
        ...
        type="button"
        disabled
        className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-2xl font-semibold transition-all border border-foreground/10 opacity-50 cursor-not-allowed glass-morphism"
      >
        <svg width="20" height="20" viewBox="0 0 24 24">
